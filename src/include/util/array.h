 ...
 * int*     int_array_push(int_array_t* list, int elt);
 ...
 * int      int_array_lastindexof(int_array_t* list, int elt);
 ...
 * int*     int_array_peekptr(int_array_t* list);
 * unsigned int_array_size(int_array_t* list);
 * void     int_array_copy(int_array_t* source, int_array_t* dest);
 * void     int_array_print(int_array_t*); // requires ARRAY_ELT_DEBUG(elt) macro to be defined
...
#ifndef ARRAY_ELT_DEBUG
#define ARRAY_ELT_DEBUG(e)
#endif
...
#define MAKE_ARRAY_CODE(T, prefix)                                  \
  ...
  int COMBINE(prefix, lastindexof)(COMBINE(prefix, t)* list, T elt) {\
    for (unsigned i=list->count; i; i--) {                          \
      if (ARRAY_EQUALS(list->list[ i-1 ], elt)) {                   \
        return (int)(i-1);                                          \
      }                                                             \
    }                                                               \
    return -1;                                                      \
